(function(global, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require("react"), require("three"), require("cannon-es")) : typeof define === "function" && define.amd ? define(["exports", "react", "three", "cannon-es"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory(global.DiceRollerReact = {}, global.React, global.THREE, global.CANNON));
})(this, function(exports2, require$$0, THREE, CANNON) {
  "use strict";
  ...
  const CANNON__namespace = /* @__PURE__ */ _interopNamespaceDefault(CANNON);
  ...
  function createD8Geometry(radius = 1) {
    ...
    faces.forEach((face) => {
      ...
      const va = new THREE__namespace.Vector3(v1[0] - v0[0], v1[1] - v0[1], v1[2] - v0[2]);
      const vb = new THREE__namespace.Vector3(v2[0] - v0[0], v2[1] - v0[1], v2[2] - v0[2]);
      const normal = new THREE__namespace.Vector3().crossVectors(va, vb).normalize();
      for (let i = 0; i < 3; i++) {
  ...
  function createD20(radius = 1, diceColor = "#2b8a51", numberColor = "#aaaaaa") {
    const geometry = new THREE__namespace.IcosahedronGeometry(radius);
    const uvs = [];
    ...
    geometry.setAttribute(
      "uv",
      new THREE__namespace.BufferAttribute(new Float32Array(uvs), 2)
    );
    const materials = [];
    for (let i = 1; i <= 20; i++) {
  ...
  function createPhysicsWorld(physicsParams) {
    ...
    const diceMaterial = new CANNON__namespace.Material("dice");
    const groundMaterial = new CANNON__namespace.Material("ground");
    const wallMaterial = new CANNON__namespace.Material("wall");
    world.addContactMaterial(new CANNON__namespace.ContactMaterial(
      diceMaterial,
      groundMaterial,
      {
    ...
    const groundShape = new CANNON__namespace.Box(new CANNON__namespace.Vec3(TABLE_CONFIG.groundSize / 2, 0.5, TABLE_CONFIG.groundSize / 2));
    const groundBody = new CANNON__namespace.Body({
      mass: 0,
      shape: groundShape,
  ...
  function createDieBody(dieType, dieSize, diceMaterial, physicsParams) {
    let dieBody;
    if (dieType === "d6") {
      const dieShape = new CANNON__namespace.Box(new CANNON__namespace.Vec3(dieSize, dieSize, dieSize));
      dieBody = new CANNON__namespace.Body({
        mass: 1,
        shape: dieShape,
      ...
      const vertices = [
        new CANNON__namespace.Vec3(1, 0, 0),
        new CANNON__namespace.Vec3(-1, 0, 0),
        new CANNON__namespace.Vec3(0, 1, 0),
        new CANNON__namespace.Vec3(0, -1, 0),
        new CANNON__namespace.Vec3(0, 0, 1),
        new CANNON__namespace.Vec3(0, 0, -1)
      ];
      ...
      const dieShape = new CANNON__namespace.ConvexPolyhedron({ vertices, faces });
      dieBody = new CANNON__namespace.Body({
        mass: 1,
        shape: dieShape,
        material: diceMaterial,
      ...
      const vertices = [
        new CANNON__namespace.Vec3(-1, t, 0),
        new CANNON__namespace.Vec3(1, t, 0),
        new CANNON__namespace.Vec3(-1, -t, 0),
        new CANNON__namespace.Vec3(1, -t, 0),
        new CANNON__namespace.Vec3(0, -1, t),
        new CANNON__namespace.Vec3(0, 1, t),
        new CANNON__namespace.Vec3(0, -1, -t),
        new CANNON__namespace.Vec3(0, 1, -t),
        new CANNON__namespace.Vec3(t, 0, -1),
        new CANNON__namespace.Vec3(t, 0, 1),
        new CANNON__namespace.Vec3(-t, 0, -1),
        new CANNON__namespace.Vec3(-t, 0, 1)
      ];
  ...
  function detectFaceValueFromBody(body, dieType = "d6") {
    const upVector = new CANNON__namespace.Vec3(0, 1, 0);
    if (dieType === "d6") {
      const faceNormals = [
        new CANNON__namespace.Vec3(1, 0, 0),
        // +X face (1)
      ...
      const faceNormals = [
        ...
        new CANNON__namespace.Vec3(0.577, 0.577, 0.577),
        new CANNON__namespace.Vec3(0.577, -0.577, 0.577),
        new CANNON__namespace.Vec3(0.577, -0.577, -0.577),
        new CANNON__namespace.Vec3(0.577, 0.577, -0.577),
  ...
  const D8_FACE_NORMALS = [
    ...
    new THREE__namespace.Vector3(0.577, 0.577, 0.577),
  ...
  function detectMeshFaceValue(mesh, dieType, currentFaceMapping) {
    const upVector = new THREE__namespace.Vector3(0, 1, 0);
    let closestFace = -1;
  ...
  function addLighting(scene) {
    const ambientLight = new THREE__namespace.AmbientLight(16777215, LIGHTING_CONFIG.ambientIntensity);
    scene.add(ambientLight);
    const directionalLight = new THREE__namespace.DirectionalLight(16777215, LIGHTING_CONFIG.directionalIntensity);
  ...
  function addGround(scene) {
    const groundGeometry = new THREE__namespace.PlaneGeometry(TABLE_CONFIG.groundSize, TABLE_CONFIG.groundSize);
    const groundVisualMaterial = new THREE__namespace.ShadowMaterial({
      opacity: 0.5
      // Adjust shadow darkness (0 = invisible, 1 = black)
    });
    const ground = new THREE__namespace.Mesh(groundGeometry, groundVisualMaterial);
  ...
  const DiceRoller = require$$0.forwardRef(({
    ...
    const cameraRef = require$$0.useRef(null);
    const worldRef = require$$0.useRef(null);
    const dieBodyRef = require$$0.useRef(null);
    const dieMeshRef = require$$0.useRef(null);
    const frameRef = require$$0.useRef(void 0);
    const controlsRef = require$$0.useRef(null);
    const materialsRef = require$$0.useRef([]);
    const baseMaterialsRef = require$$0.useRef([]);
    ...
    const [physicsParams, setPhysicsParams] = require$$0.useState(DEFAULT_PHYSICS);
    const [worldReady, setWorldReady] = require$$0.useState(false);
    ...
    const initScene = require$$0.useCallback(() => {
    ...
    const createDie = require$$0.useCallback(() => {
    ...
    require$$0.useEffect(() => {
      ...
      if (groundMaterial) {
        worldRef.current.addContactMaterial(new CANNON__namespace.ContactMaterial(
          diceMaterial,
          groundMaterial,
          {
            friction: physicsParams.diceFriction,
    ...
    const handlePhysicsChange = require$$0.useCallback((newParams) => {
      if (isRolling) {
  ...
  exports2.DiceRoller = DiceRoller;
  exports2.THROW_CONFIG = THROW_CONFIG;
  Object.defineProperty(exports2, Symbol.toStringTag, { value: "Module" });
});
//# sourceMappingURL=dice-roller-react.umd.js.map
